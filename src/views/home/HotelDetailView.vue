  ...
  <div class="hotel-detail-page bg-light min-vh-100">
    ...
    <main v-if="hotel" class="container pt-5 mt-5 pb-5">
        ...
        <div class="col-lg-8">
          ...
          <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <span class="text-muted fw-medium"
              ><i class="fas fa-map-marker-alt me-1"></i> {{ hotel.hotel_city }}</span
            >
          </div>
          ...
          <div class="glass-panel p-4 sticky-top" style="top: 100px; border-radius: 20px">
            ...
            <div class="text-center">
              <p class="small text-muted mb-3 italic">
                "Trải nghiệm sự tiện nghi bậc nhất tại {{ hotel.hotel_name }}"
              </p>
              <button @click="scrollToRooms" class="btn btn-premium w-100 py-3">
                Xem phòng trống
              </button>
            </div>
          </div>
      ...
      <section id="rooms-section" class="mt-5">
        <h2 class="display-6 fw-bold mb-4" style="font-family: var(--font-serif)">
          Phòng Nghỉ & Bảng Giá
        </h2>

        <!-- Date Filter Card -->
        <div
          class="glass-panel p-4 mb-5 shadow-soft d-flex align-items-end justify-content-between flex-wrap gap-4"
          style="border-radius: 20px; border-left: 5px solid var(--color-gold) !important"
        >
          <div class="flex-grow-1" style="min-width: 200px">
            <label class="form-label small fw-bold text-uppercase text-muted mb-2"
              ><i class="fas fa-calendar-check text-gold me-2"></i>Ngày nhận</label
            >
            <input
              id="detail-checkin"
              type="text"
              class="form-control border-light bg-white shadow-sm py-2"
          ...
          <div class="flex-grow-1" style="min-width: 200px">
            <label class="form-label small fw-bold text-uppercase text-muted mb-2"
              ><i class="fas fa-calendar-times text-gold me-2"></i>Ngày trả</label
            >
            <input
              id="detail-checkout"
              type="text"
          ...
          <div class="flex-grow-1" style="min-width: 150px">
            ...
            <select
              v-model="filterType"
              class="form-select border-light bg-white shadow-sm py-2"
          ...
          <div>
            <button
              @click="fetchHotel"
              class="btn btn-premium px-4 py-2 mb-1"
        ...
        <div
          v-if="hotel.rooms.length === 0"
          class="text-center py-5 text-danger glass-panel"
          style="border-radius: 20px"
        >
          <i class="fas fa-exclamation-circle display-1 opacity-25 mb-3"></i>
          <h4>Rất tiếc! Hiện tại không có phòng nào khả dụng.</h4>
          <p class="text-muted">
            Vui lòng thử lại với khoảng thời gian khác hoặc liên hệ khách sạn.
          </p>
        </div>
        ...
        <div v-else class="row g-4">
          <div v-for="room in hotel.rooms" :key="room.id" class="col-md-12">
            <div
              class="card border-0 shadow-soft overflow-hidden hover-lift"
                ...
                <div class="col-lg-8 card-body p-4 d-flex flex-column justify-content-between">
                  <div>
                    ...
                    <div class="d-flex flex-wrap gap-3 mb-4">
                      <div
                        class="d-flex align-items-center gap-2 small text-muted bg-light px-3 py-1 rounded-pill"
                      >
                    ...
                    <div class="d-flex align-items-center gap-3">
                      <div class="d-flex flex-column align-items-end">
                        ...
                        <div class="guest-input-group">
                          <select
                            v-model.number="roomGuests[room.id]"
                            class="form-select form-select-sm"
                            style="border-radius: 8px; max-width: 100px"
                          >
                            <option v-for="g in getGuestOptions(room)" :key="g" :value="g">
                              {{ g }}
                            </option>
                          </select>
                        </div>
                      ...
                      <button
                        ...
                        class="btn btn-premium d-flex align-items-center gap-2 px-4 py-3 h-100"
                        :disabled="isAdding === room.id"
                      >
                        <span
                          v-if="isAdding === room.id"
      ...
      <section class="mt-5 pt-5 border-top">
        <div class="d-flex align-items-center justify-content-between mb-4">
          <h2 class="display-6 fw-bold mb-0" style="font-family: var(--font-serif)">
          ...
          <div class="d-flex align-items-center gap-2">
            ...
            <div class="text-gold small">
              ...
              <i
                ...
                v-for="n in 5"
                :key="n"
                :class="{ 'opacity-25': n > Math.round(hotel.average_rating || 5) }"
              ></i>
            </div>
        ...
        <div
          v-if="!hotel.feedbacks || hotel.feedbacks.length === 0"
        ...
        <div v-else class="row g-4">
          <div v-for="fb in hotel.feedbacks" :key="fb.id" class="col-md-6">
            <div
              class="glass-panel p-4 h-100 shadow-soft border-0"
              style="border-radius: 20px; background: white"
            >
              <div class="d-flex align-items-center gap-3 mb-3">
                <div
                  class="user-avatar bg-gold text-dark d-flex align-items-center justify-content-center fw-bold rounded-circle"
                  style="
                    width: 48px;
                    height: 48px;
                    min-width: 48px;
              ...
              <p class="text-muted mb-0 italic" style="font-size: 0.95rem; line-height: 1.6">
                "{{ fb.content }}"
              </p>
            </div>
          </div>
    ...
    <div
      v-else-if="error"
      class="min-vh-100 d-flex flex-column align-items-center justify-content-center text-center p-4"
    >
      <div class="mb-4">
  ...
  const initPickers = () => {
    ...
    flatpickr('#detail-checkout', {
      ...
      onChange: (selectedDates) => {
        if (selectedDates.length > 0) {
          const d = selectedDates[0]
          checkoutDate.value = new Date(d.getTime() - d.getTimezoneOffset() * 60000)
            .toISOString()
            .split('T')[0]
          datesSelected.value = !!(checkinDate.value && checkoutDate.value)
        }
...
const getGuestOptions = (room) => {
  const max = Math.max(1, parseInt(room?.capacity || 1, 10))
  return Array.from({ length: max }, (_, i) => i + 1)
}

const normalizeGuests = (room, rawValue) => {
  const max = Math.max(1, parseInt(room?.capacity || 1, 10))
  let value =
    rawValue !== undefined ? parseInt(rawValue, 10) : parseInt(roomGuests[room.id] || 1, 10)
  if (Number.isNaN(value)) value = 1
  if (value < 1) value = 1
  if (value > max) value = max
  roomGuests[room.id] = value
  return value
}
