  ...
  <div class="checkout-page bg-light min-vh-100 pb-5">
    ...
    <main class="container pt-5 mt-5">
      ...
      <div v-else class="row g-5">
          ...
          <div class="glass-panel p-5 bg-white shadow-soft" style="border-radius: 24px;">
            ...
            <form id="checkout-form" class="row g-4">
              <div class="col-md-12">
                ...
                <input v-model="form.guest_name" type="text" class="form-control form-control-lg border-light bg-light" placeholder="Nguyễn Văn A" required>
              </div>
              <div class="col-md-6">
                <label class="form-label small fw-bold text-muted text-uppercase">Email liên hệ</label>
                <input v-model="form.guest_email" type="email" class="form-control form-control-lg border-light bg-light" placeholder="email@vi-du.com" required>
              </div>
              <div class="col-md-6">
                <label class="form-label small fw-bold text-muted text-uppercase">Số điện thoại</label>
                <input v-model="form.guest_phone" type="tel" class="form-control form-control-lg border-light bg-light" placeholder="09xxxxxxxx" required>
              </div>
              ...
              <div class="col-12">
                <div class="row g-3">
                  <div class="col-md-6">
                    <label class="payment-card-option p-4 d-flex align-items-center gap-3 cursor-pointer" :class="{active: form.payment_method === 'vnpay'}">
                      <input type="radio" v-model="form.payment_method" value="vnpay" class="d-none">
                      <img src="https://static.vnpay.vn/registration/images/vnpay-logo-2.61f71a93.svg" height="24" alt="VNPAY">
                      <span class="fw-bold fs-5">VNPAY QR</span>
                    </label>
                  </div>
                  <div class="col-md-6">
                    <label class="payment-card-option p-4 d-flex align-items-center gap-3 cursor-pointer" :class="{active: form.payment_method === 'offline'}">
                      <input type="radio" v-model="form.payment_method" value="offline" class="d-none">
                      <div class="icon-box bg-gold text-white p-2 rounded-circle d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
        ...
        <!-- Order Summary -->
        <div class="col-lg-5">
          <div class="glass-panel p-4 bg-white shadow-soft sticky-top" style="top: 100px; border-radius: 24px;">
            <h4 class="fw-bold mb-4">Danh sách phòng</h4>
            <div class="order-items-list mb-4 overflow-auto" style="max-height: 300px;">
               <div v-for="item in cartStore.items" :key="item.id" class="d-flex align-items-center gap-3 mb-3 pb-3 border-bottom border-light">
                 <img :src="item.room?.hotel_image || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=500'" class="rounded-3" style="width: 60px; height: 60px; object-fit: cover;">
                 <div class="flex-grow-1">
                    ...
                    <div class="d-flex flex-wrap gap-2 text-muted" style="font-size: 0.7rem;">
                       <span class="bg-light px-2 py-1 rounded"><i class="fas fa-calendar-alt text-gold me-1"></i> {{ item.checkin }} - {{ item.checkout }}</span>
                       <span class="bg-light px-2 py-1 rounded"><i class="fas fa-user-friends text-gold me-1"></i> {{ item.number_of_guests || 1 }} khách</span>
                    </div>
                 </div>
            ...
            <div class="d-flex flex-column gap-3 mb-4">
              ...
              <div class="d-flex justify-content-between fw-bold fs-3 text-dark pt-3 border-top border-2 border-gold-subtle">
                <span>Tổng cộng</span>
                <span>{{ formatPrice(cartStore.totalPrice * 1.1) }}₫</span>
              </div>
            </div>

            <button @click="processBooking" class="btn btn-premium btn-lg w-100 py-3 mt-2 shadow-sm d-flex align-items-center justify-content-center gap-2" :disabled="isProcessing">
            ...
            <p class="text-center text-muted small mt-4 mb-0">Bằng việc bấm nút, bạn đồng ý với các Điều khoản & Chính sách của StayGo.</p>
          </div>
        </div>
      </div>
    </main>
...
const processBooking = async () => {
  ...
  try {
    ...
    const bookingRes = await api.post('/bookings', {
      ...
      total_price: cartStore.totalPrice * 1.1
    });
